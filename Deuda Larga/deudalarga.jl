...
function switch_CE_costs!(dd::Default, d0 = -0.18819, d1 = 0.24558)
    dd.pars[:d0] = d0
    dd.pars[:d1] = d1
    nothing
end

function switch_Arellano_costs!(dd::Default)
    dd.pars[:defcost_OG] = 1
    nothing
end

function switch_linear_costs!(dd::Default, Δ = dd.pars[:Δ])
    dd.pars[:defcost_OG] = 0
    dd.pars[:d1] = 0
    dd.pars[:d0] = Δ
    nothing
end
...
function DeudaLarga(;
    ...
    pars = Dict(:β => β, :γ => γ, :r => r, :ψ => ψ, :χ => χ, :Δ => Δ, :ρy => ρy, :σy => σy, :κ => κ, :ρ => ρ, :ℏ => ℏ, :defcost_OG => defcost_OG)
    ...
    return DeudaLarga(pars, bgrid, ygrid, Py, v, vR, vD, prob, gc, gb, q)
end
...
function q_iter!(new_q, dd::DeudaLarga)
    ...
    itp_q = make_itp(dd, dd.q, 1) # para repago
    itp_qD= make_itp(dd, dd.q, 2) # para default
    ...
    for (jbp, bpv) in enumerate(dd.bgrid), jy in eachindex(dd.ygrid)
        ...
        for (jyp, ypv) in enumerate(dd.ygrid)
            ...
            rep_R = (1 - prob_def) * R + prob_def * (1 - ℏ) * itp_qD((1-ℏ)*bpv, ypv)
